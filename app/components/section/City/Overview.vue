...
const hasHighlights = computed(
  () => (props.city.experience_highlights?.length ?? 0) > 0,
);
...
const infoItems = computed(() => {
  ...
  if (props.city.region)
    items.push({ label: t("cityOverview.region"), value: props.city.region });
  if (props.city.population)
    items.push({
      label: t("cityOverview.population"),
      value: props.city.population,
    });
  if (props.city.founded)
    items.push({ label: t("cityOverview.founded"), value: props.city.founded });
  if (props.city.unesco_status)
    items.push({
      label: t("cityOverview.unescoStatus"),
      value: props.city.unesco_status,
    });
  if (props.city.best_season)
    items.push({
      label: t("cityOverview.bestSeason"),
      value: props.city.best_season,
    });
  return items;
});
...
<template>
  <section
    class="bg-orange-cream py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6"
  >
    <div class="container mx-auto">
      ...
      <!-- Tabs -->
      <div
        class="border-b border-dark-light mb-8 sm:mb-10 overflow-x-auto scrollbar-hide"
      >
      ...
      <div class="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <template v-if="activeTab === 0">
            <div class="mb-6 sm:mb-8">
              ...
              <h5
                ...
                class="text-grey-darker text-xs sm:text-base mb-0.5"
            ...
            <div
              ...
              class="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-4 sm:gap-y-5"
            >
              <div v-for="item in infoItems" :key="item.label">
                <p class="text-grey-darker text-xs sm:text-sm mb-0.5">
                  {{ item.label }}:
                </p>
                <p
                  class="text-dark-normal text-sm sm:text-base font-semibold leading-snug"
                >
                  {{ item.value }}
                </p>
              </div>
        ...
        <div v-if="photos.length" class="w-full lg:w-1/2 h-full">
          <div
            class="relative rounded-2xl sm:rounded-3xl overflow-hidden h-[240px] sm:h-[300px] md:h-[360px] lg:h-full lg:min-h-[320px]"
          >
            <NuxtImg
      ...
      <div v-if="hasHighlights" class="mt-10 sm:mt-12 md:mt-14">
        <div class="flex items-center justify-between mb-5 sm:mb-6">
          ...
          <div v-if="needsSwiper" class="flex gap-2">
            <button
              ...
              :class="
                canScrollLeft
                  ? 'hover:bg-dark-light text-dark-normal'
                  : 'text-grey-normal cursor-not-allowed'
              "
        ...
        >
          ...
          >
            ...
            <h4
              class="text-sm sm:text-base font-bold text-dark-normal mb-1.5 sm:mb-2"
            >
              {{ hl.title }}
            </h4>
            <p
              class="text-grey-darker text-xs sm:text-sm leading-relaxed line-clamp-3"
            >
